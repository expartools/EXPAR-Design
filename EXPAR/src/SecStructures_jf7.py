...
def complement(dna):
    t=maketrans("AGCTagct",	"TCGAtcga") # to make the translation table
...
class SecStructures(object):
    ...
    computes sets of self-dimers, and (in case of two sequences) cross-dimers.
    ...
    def _format_str(self, full=True):
        """If 'full', for single sequence output self-dimers
        For two sequences output only cross-dimers
        If not 'full', for single sequence output COUNTS and min dG of self-dimers
    ...
    def _recalculate(self):
        """For single sequence calculate self-dimers.
...
#        structure_string += '\n'
        structure_string="%(spacerM)s%(matches)s\n" % {'spacerM':spacerM,'matches':matches}
        structure_string += 'number of matches is %d ' % structure_string.count('|')
        structure_string=str(structure_string.count('|'))
...
if __name__ == "__main__":
	for ET_record in SeqIO.parse('example.txt', "fasta"):
		structs = SecStructures(SeqRecord(Seq('ATTACTACTTTAATATATATTTACTACT')))
		#print 'seq1 self dimer',str(structs).split()[0]
	print str(structs).split()[0]
